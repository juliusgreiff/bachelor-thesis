...
VaR_eGARCH_Sim4 <- matrix(data = NA,nrow = n.sim,ncol = 3)
ES_eGARCH_Sim4 <- matrix(data = NA, nrow = n.sim,ncol = 3)
...
for(i in 1:n.sim){
  ...
  VaR_eGARCH_Sim4[i,] <- c(sVaR - true_VaR, gjrVaR - true_VaR, eVaR - true_VaR)
  ES_eGARCH_Sim4[i,] <- c(sES - true_ES , gjrES - true_ES , eES - true_ES )
}

df_Error_eGARCH_Sim4 <- data.frame(matrix(nrow = 2,ncol = 3))
df_Error_eGARCH_Sim4[1,] <- sqrt(colMeans(VaR_eGARCH_Sim4^2))
df_Error_eGARCH_Sim4[2,] <- sqrt(colMeans(ES_eGARCH_Sim4^2))

print(df_Error_eGARCH_Sim4)
