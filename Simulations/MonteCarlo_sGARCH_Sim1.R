...
#Initialization of matrices for storing risk measure values
VaR_sGARCH_Sim1 <- matrix(data = NA,nrow = n.sim,ncol = 3)
ES_sGARCH_Sim1 <- matrix(data = NA, nrow = n.sim,ncol = 3)

#Setting the parameter combination
pars <- list(mu = 0,
             omega = 0.5,
             alpha1 = 0.3,
             beta1 = 0.2)
...
for(i in 1:n.sim){
  ...
  #Estimation of true risk measure values
  true_VaR <- garch@path$sigmaSim[n]*VaR(qnorm)
  true_ES <- garch@path$sigmaSim[n]*ES(qnorm)
  
  #Simulated historical loss data
  L <- as.numeric(garch@path$seriesSim)
  
  #Recursive calculation of conditional standard deviation and subsequent risk measures
  ...
  VaR_sGARCH_Sim1[i,] <- c(sVaR - true_VaR, gjrVaR - true_VaR, eVaR - true_VaR)
  ES_sGARCH_Sim1[i,] <- c(sES - true_ES , gjrES - true_ES , eES - true_ES )

}

#Calculation and storage of RMSE values
df_Error_sGARCH_Sim1 <- data.frame(matrix(nrow = 2,ncol = 3))
df_Error_sGARCH_Sim1[1,] <- sqrt(colMeans(VaR_sGARCH_Sim1^2))
df_Error_sGARCH_Sim1[2,] <- sqrt(colMeans(ES_sGARCH_Sim1^2))

print(df_Error_sGARCH_Sim1)
