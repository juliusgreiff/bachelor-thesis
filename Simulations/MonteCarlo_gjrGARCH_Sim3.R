...
VaR_gjrGARCH_Sim3 <- matrix(data = NA,nrow = n.sim,ncol = 3)
ES_gjrGARCH_Sim3 <- matrix(data = NA, nrow = n.sim,ncol = 3)
...
specs <- ugarchspec(variance.model = list(model = "gjrGARCH"),
...
for(i in 1:n.sim){
  ...
  VaR_gjrGARCH_Sim3[i,] <- c(sVaR - true_VaR, gjrVaR - true_VaR, eVaR - true_VaR)
  ES_gjrGARCH_Sim3[i,] <- c(sES - true_ES , gjrES - true_ES , eES - true_ES )
  
}

df_Error_gjrGARCH_Sim3 <- data.frame(matrix(nrow = 2,ncol = 3))
df_Error_gjrGARCH_Sim3[1,] <- sqrt(colMeans(VaR_gjrGARCH_Sim3^2))
df_Error_gjrGARCH_Sim3[2,] <- sqrt(colMeans(ES_gjrGARCH_Sim3^2))

print(df_Error_gjrGARCH_Sim3)
